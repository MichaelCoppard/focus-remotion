/**
 * InfluencerVsSystem — [1:40-2:00]
 * "INFLUENCER" struck through → "SYSTEM" in gold.
 * Label: "THE GOAL ON DAY ONE"
 */
...
export const InfluencerVsSystem: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  const easeOut = (t: number) => 1 - Math.pow(1 - t, 3);

  // ── "INFLUENCER" springs in ───────────────────────────────────────────────
  ...
  return (
    <GraphicWrapper>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        ...
        {/* ── INFLUENCER (dimmed + strikethrough) ── */}
        <div style={{
          position: "relative",
          opacity: topOp,
