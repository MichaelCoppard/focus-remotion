/**
 * ProductiveFailure — [0:40-1:05]
 * Coined term card. Ugly first versions build more efficient systems.
 */
...
export const ProductiveFailure: React.FC = () => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();

  const easeOut = (t: number) => 1 - Math.pow(1 - t, 3);

  // ── "PRODUCTIVE" slides down ──────────────────────────────────────────────
  ...
  // ── "FAILURE" springs up with bounce ─────────────────────────────────────
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
        ...
        <div style={{
          ...
          <span style={{
            ...
            Ugly first versions build more efficient systems
          </span>
        </div>
